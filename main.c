...
int motion_detected ;  // variable to keep track of motion detection
int timer = 0; // variable to keep track of the timer
bool turn_off_gpios = false;
...
#define CENTRAL_SCANNING_LED            23                    /**< Scanning LED will be on when the device is scanning. */
#define CENTRAL_CONNECTED_LED           22                 /**< Connected LED will be on when the device is connected. */
#define LEDBUTTON_LED                   14            /**< LED to indicate a change of state of the the Button characteristic on the peer. */
#define LEDBUTTON_LED_1                   15            /**< LED to indicate a change of state of the the Button characteristic on the peer. */
#define LEDBUTTON_LED_2                   30            /**< LED to indicate a change of state of the the Button characteristic on the peer. */
#define LEDBUTTON_LED_3                   31            /**< LED to indicate a change of state of the the Button characteristic on the peer. */

#define LEDBUTTON_BUTTON          BSP_BUTTON_0                          /**< Button that writes to the LED characteristic of the peer. */
#define BUTTON_DETECTION_DELAY    APP_TIMER_TICKS(500)                   /**< Delay from a GPIOTE event until a button is reported as pushed (in number of timer ticks). */
    ...
    switch (p_lbs_c_evt->evt_type)
    {
        ...
        case BLE_LBS_C_EVT_BUTTON_NOTIFICATION:
        {
            ...
            if (p_lbs_c_evt->params.button.button_state)
            {
                ...
                nrf_gpio_pin_set(LEDBUTTON_LED_1);
                nrf_gpio_pin_set(LEDBUTTON_LED_2);
                nrf_gpio_pin_set(LEDBUTTON_LED_3);
            }
            else
            {
                ...
                nrf_delay_ms(1000);  // wait for 1 second
                   if (timer == 4) {  // check if 5 seconds have passed
                   turn_off_gpios = true;
                   motion_detected = false;  // set motion_detected as False
                timer=0;
                }
                }
        }
        ...
        nrf_gpio_pin_clear(LEDBUTTON_LED_1);
        nrf_gpio_pin_clear(LEDBUTTON_LED_2);
        nrf_gpio_pin_clear(LEDBUTTON_LED_3);
        turn_off_gpios = false;  // reset flag
...
int main(void)
{
    ...
    nrf_gpio_cfg_output(LEDBUTTON_LED_1);
    nrf_gpio_cfg_output(LEDBUTTON_LED_2);
    nrf_gpio_cfg_output(LEDBUTTON_LED_3);
    // Initialize.
    log_init();
    timer_init();
    leds_init();
